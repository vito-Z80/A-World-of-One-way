...
spriteId	byte	
direction	byte	
launchTime	byte 	; врем€ до начала движени€ объекта
...
exec		dw 	; The address of the procedure executed every frame for the current object. #0000 = not called.
		; пор€док следующих 6ти байт нельз€ мен€ть !!!
scrAddrL:	byte 
scrAddrH:	byte
sprAddrL:	byte
sprAddrH:	byte
...
clearSide:	byte 	; сторона с которой требуетс€ отчистка хвоста спрайта. 0 = не чистить
clrScrAddrL:	byte 	; адрес экрана где будет происходить отчистка.
clrScrAddrH:	byte 	; 

		; ввести переменную drawOnce ? к примеру дл€ двери выхода - это объект, но нет смысла ее печатать каждый кадр
		; достаточно при инициализации уровн€.
